...
case class FetchNamePlanner(studentId: Int, override val planContext: PlanContext) extends Planner[String] {
  override def plan(using planContext: PlanContext): IO[String] = {
    ...
    readDBRows(
      ...
      case row :: _ =>
        row.asObject.flatMap(_.apply("name").flatMap(_.asString)) match {
