...
case class AssignDepartmentPlanner(student_id: Int, department: String, override val planContext: PlanContext) extends Planner[String] {
    ...
    checkStudentExists.flatMap{ exists =>
      if (!exists) {
        ...
        val assignDepartment = writeDB(
          s"UPDATE ${schemaName}.${schemaName} SET department = ? WHERE student_id = ?".stripMargin,
          List(
            SqlParameter("TEXT", department),
            SqlParameter("Int", student_id.toString)
          )
        )
        assignDepartment.flatMap { _ =>
          IO.pure("Department assigned successfully")
        }
      }
    }
  }
