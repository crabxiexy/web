...
object Init {
  def init(config:ServerConfig):IO[Unit]=
    ...
    for{
      ...
      _ <- writeDB(s"CREATE TABLE IF NOT EXISTS ${schemaName}.HW (activity_id INT, student_id INT, TA_id INT, submitTime INT, imgUrl TEXT, is_checked INT, response TEXT) ", List())
