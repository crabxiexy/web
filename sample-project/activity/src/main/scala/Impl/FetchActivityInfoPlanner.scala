    ...
    readDBRows(sqlQuery, List(SqlParameter("Int", activityID.toString))).flatMap { rows =>
      rows.headOption match {
        case Some(json) =>
          ...
          val startTime = json.hcursor.downField("startTime").as[OffsetDateTime].getOrElse(OffsetDateTime.now())
          val finishTime = json.hcursor.downField("finishTime").as[OffsetDateTime].getOrElse(OffsetDateTime.now())
